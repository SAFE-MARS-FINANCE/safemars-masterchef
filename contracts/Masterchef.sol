...
import './interfaces/ISmars.sol';
import './interfaces/INativeTokenHolderVault.sol';
import './utils/SafeBEP20.sol';

// MasterChef is the master of Smars. He can make Smars and he is a fair guy.
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Smars is sufficiently
...
contract Masterchef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. Smars to distribute per block.
        uint256 lastRewardBlock;  // Last block number that Smars distribution occurs.
        uint256 accSmarsPerShare;   // Accumulated Smars per share, times 1e12. See below.
    ...
    // Smars tokens created per block.
    uint256 public marsPerBlock;
    // Bonus muliplier for early smars makers.
    ...
    // The block number when Smars mining starts.
    ...
    // Update the given pool's Smars allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending Smars on frontend.
    function pendingMars(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accSmarsPerShare = pool.accSmarsPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 marsReward = multiplier.mul(marsPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accSmarsPerShare = accSmarsPerShare.add(marsReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accSmarsPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 marsReward = multiplier.mul(marsPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        smars.mint(devaddr, marsReward.div(10));
        smars.mint(address(this), marsReward);
        pool.accSmarsPerShare = pool.accSmarsPerShare.add(marsReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for Smars allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accSmarsPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accSmarsPerShare).div(1e12);
    ...
    // Safe smars transfer function, just in case if rounding error causes pool to not have enough Smars.
    function safeMarsTransfer(address _to, uint256 _amount) internal {
        uint256 marsBal = smars.balanceOf(address(this));
        if (_amount > marsBal) {
            smars.transfer(_to, marsBal);
        } else {
            smars.transfer(_to, _amount);
        }
    }
